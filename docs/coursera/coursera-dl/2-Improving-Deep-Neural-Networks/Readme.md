...
- Another idea to get the bias /  variance if you don't have a 2D plotting mechanism:
  ...
  - The human error are also called Bayesian optimum error. It might differ and not always optimally 0% in all cases.
...
- In the modern deep learning, big data era, so long as you can keep training a bigger network, almost always just **reduces your bias without necessarily hurting your variance**, so long as you regularize appropriately. 
- Getting more data pretty much always **reduces your variance and doesn't hurt your bias much.**
- Two big reasons that deep learning has been so useful for supervised learning, because there's much less of this tradeoff.
...
- Can't rely on any one feature because any one feature could go away at random.
...
- If you implement the formula as written here, you won't actually get the green curve when Beta equals 0.98, you actually get the purple curve here. 
- ![](Images/5.png)
...
- To solve the bias issue we have to use this equation:
  ...
  ```
- ![](Images/7.png)
- As t becomes large, Beta to the t will approach 0, which is why when t is large enough, the bias correction makes almost no difference. This is why when t is large, the purple line and the green line pretty much overlap.
...
- SdW will be relatively small, so we're dividing dW by relatively small number for greater updates in horizontal direction (towards minima). 
- Sdb will be relatively large, so that here we're dividing db by relatively large number in order to slow down the updates on a vertical dimension.
...
- Our NN parameters will be:
  - `W[1]`, `b[1]`, ..., `beta[1]`, `gamma[1]`, `W[L]`, `b[L]`,  ..., `beta[L]`, `gamma[L]`
...
- The first reason is the same reason as why we normalize X i.e. it speeds up learning process.
- The second reason is that batch normalization reduces the problem of input values changing (shifting) in the hidden layers.
  - Suppose you trained your data sets on all images of black cats.
  - If you try to apply this network to data with colored cats where the positive examples are not just black cats, then your cosfa might not do very well. This data distribution changing goes by the somewhat fancy name, **covariate shift**.
  ...
  - Batch norm does, reduces the amount that the distribution of these hidden unit values shifts around.
...
- We can use the exponential weighted average across the mini-batches.
- We will use the estimated values of the mean and variance to test.
- This method is also sometimes called "Running average". Suppose take average of (mu)l parameter for every (l)th layer in each of the mini batches, similarly for (beta).
...
- Softmax activation equations:
  ...
  ```
![](Images/softmax.png)
- The unusual thing about softmax activation function g() is that, it takes input a 4 by 1 vector and it outputs a 4 by 1 vector. Unlike activation functions like ReLu, Sigmoid, tanH which takes input a real number and output a real number. 
...
- The loss function used with softmax:
  - ![](Images/loss_softmax.png)
  - Only y2nd term(correct classification) because rest are zero, to minimise Loss L(y,yhat) ==> reduce -log(y2hat) ==> increase y2hat probability!
  ```
